...
class Alphabet {     // e.g. S = 5 for DNA (A,C,G,T + $)
   ...
   public:
    ...
    Alphabet(const std::string& T) {
        ...
        for (char c : T) charCounts[(unsigned char)c]++;
     ...
     */
    Alphabet(const std::vector<length_t>& charCounts) { initialize(charCounts); }
     ...
     */
    bool inAlphabet(char c) const { return charToIndex[(unsigned char)c] >= 0; }
     ...
     */
    std::array<char, S - 1> operator()() const { return redChar; }
